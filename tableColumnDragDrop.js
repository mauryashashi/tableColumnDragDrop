angular.module('tableColumnDragDropModule', [])
    ...
    .directive('tableColumnDragDrop', function () {
        return {
            link: function (scope, element, attrs) {
                ...
                if (attrs.handle && element.find(attrs.handle).length<1 ){
                    $handle = angular.element('<span class="handle"><i class="fa fa-caret-right"></i><i class="fa fa-caret-left"></i></span>');
                ...
                var ghostWrapperHtml = '<div class="ghost bg-warning" style="position: fixed;"><div style="zoom:80%">{ghostHtml}</div></div>';
                ...
                var clearGhostElements = function () {
                    element.closest("table").find('.ghost__placeholder').removeClass('ghost__placeholder').removeAttr('width');
                    //angular.element('.ghost__placeholder').removeClass('ghost__placeholder').removeAttr('width');
                    element.closest("table").find('.ghost__data').removeClass('ghost__data');
                ...
                var ghostTable = function (direction, index,clearlastmove) {
                    // check if last know direction and current direction
                    // and last know index and current index are the same
                    // if so, prevent action
                    if (direction === lastDirection && index === lastIndex) {
                        return false;
                    }

                    // find last ghost placeholder
                    var $ghostPlaceholder = element.closest("table").find('.ghost__placeholder').last();
                    //angular.element('.ghost__placeholder').last();

                    // find current table data elements
                    var $currentElements = element.closest("table").find('tbody td:nth-child(' + (index) + ')')
                    // angular.element('tbody td:nth-child(' + (index) + ')');

                    // find ghost data elements
                    var $ghostElements =element.closest("table").find('.ghost__data');
                    //angular.element('.ghost__data');
